...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def compute_evaluation_metrics(self, returns_all: Dict):
        metrics_results = {}
        horizon_metrics = {metric_name: [] for metric_name in self.metrics.keys()}
        
        for i in range(returns_all['prediction'].shape[1]):  # 遍历所有预测步长
            ...
            for metric_name, metric_func in self.metrics.items():
                ...
                horizon_metrics[metric_name].append(metric_item.item())
            self.logger.info(f'Horizon {i + 1:02d}{metric_repr}')
        ...
        for metric_name, metric_func in self.metrics.items():
            ...
            horizon_metrics[metric_name] = sum(horizon_metrics[metric_name]) / len(horizon_metrics[metric_name])
        
        avg_repr = ''
        for metric_name in self.metrics.keys():
            avg_repr += f', {metric_name}: {horizon_metrics[metric_name]:.4f}'
        self.logger.info(f'Average over 12 horizons{avg_repr}')
