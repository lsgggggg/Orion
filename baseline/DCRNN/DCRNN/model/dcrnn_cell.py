...
class DCGRUCell(rnn_cell_impl.RNNCell):
    ...
    def __init__(self, num_units, adj_mx, max_diffusion_step, num_nodes, num_proj=None,
        ...
        if np.any(np.isnan(adj_mx)) or np.any(np.isinf(adj_mx)):
        ...
        if filter_type == "laplacian":
            ...
            supports.append(self._random_walk(adj_mx).T)
        elif filter_type == "dual_random_walk":
            supports.append(self._random_walk(adj_mx).T)
            supports.append(self._random_walk(adj_mx.T).T)
        else:
    ...
    def _random_walk(self, adj_mx):
        # 计算出度矩阵 DO
        d_out = np.sum(adj_mx, axis=1)
        # 检查是否存在全 0 行
        if np.any(d_out == 0):
            print("Warning: Found nodes with out-degree 0, adding small epsilon to avoid division by zero.")
            d_out[d_out == 0] = 1e-10  # 添加小值以避免除以 0
        d_out_inv = 1.0 / d_out
        d_out_inv = np.diag(d_out_inv)
        transition = np.dot(d_out_inv, adj_mx)
        # 检查转移矩阵是否存在 nan 或 inf
        if np.any(np.isnan(transition)) or np.any(np.isinf(transition)):
            raise ValueError("Transition matrix contains NaN or Inf values!")
        return transition

    @staticmethod
    def _build_sparse_matrix(L):
        # 检查L是否为numpy数组，如果是则转换为scipy稀疏矩阵
        if isinstance(L, np.ndarray):
            import scipy.sparse as sp
            L = sp.csr_matrix(L)
        # 现在L应该是scipy稀疏矩阵，可以安全调用tocoo()
        L = L.tocoo()
