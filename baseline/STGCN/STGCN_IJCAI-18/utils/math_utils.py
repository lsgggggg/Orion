...
def z_inverse(x, mean, std):
    ...
    '''
    mean_flow = mean[0, 0, 0, 0]  # Shape: scalar
    std_flow = std[0, 0, 0, 0]    # Shape: scalar
    return x * std_flow + mean_flow

def MAPE(v, v_, threshold=1.0):
    '''
    Mean absolute percentage error.
    :param v: np.ndarray or int, ground truth.
    :param v_: np.ndarray or int, prediction.
    :param threshold: float, ignore ground truth values below this threshold to avoid division by near-zero.
    :return: int, MAPE averages on all elements of input.
    '''
    mask = v > threshold
    if not np.any(mask):
        logger.warning("All ground truth values are below threshold; returning MAPE as 0.")
        return 0.0
    v_filtered = v[mask]
    v_filtered_ = v_[mask]
    return np.mean(np.abs(v_filtered_ - v_filtered) / (v_filtered + 1e-5))
...
def evaluation(y, y_, x_stats):
    ...
    if dim == 3:
        ...
        return np.array([MAPE(v, v_, threshold=1.0), MAE(v, v_), RMSE(v, v_)])
    else:
        # multi_step case
        tmp_list = []
